...
from core.db_settings import fetch_query
from core.db_settings import execute_query
logger = logging.getLogger(__name__)

from datetime import datetime

async def order(user) -> None:
    """
    ...
    try:
        ...
        query2 = """
            INSERT INTO orders (user_id, menu_id, amount, status, order_type, created_at)
            VALUES ($1, $2, $3, $4, $5, $6) RETURNING id
        """
        params= (user['id'],menu_id, amount,  status,  order_type, datetime.now())
        order_id = await fetch_query(query=query2, params=params, fetch="one")
...
async def show_all_products() -> None:
    ...
    products = await fetch_query(query=query, fetch="all")
    if not products:
...
async def show_orders_by_time() -> None:
    ...
    time2 = datetime.strptime(time, "%Y-%m-%d").date()
    query = "SELECT * FROM orders WHERE created_at = $1"
    params = (time2,)
    orders = await fetch_query(query=query, params=params, fetch="all")
    if not orders:
